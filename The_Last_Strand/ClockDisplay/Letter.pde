...
class Letter {
  ...
  Letter(char initCharacter, PVector charPos, float seperation, PVector field, PImage background) {
    this.field = field;
  ...
  void changeChar(char newCharacter) {
    ...
    while(newBallCount != balls.size()) {
      ...
      if(balls.size() == 0) {
        balls.add(new Ball(charSize.copy().div(2).add(charPos), size, field, background));
      }
      ...
      if(newBallCount > balls.size()) {
        balls.add(int(random(0, balls.size()-1)), new Ball(balls.get(int(random(0, balls.size()-1))).currPos.copy(), size, field, background));
      }
