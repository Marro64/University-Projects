class SensorSmooth {
  ...
  void debugDisplay(int fontSize) {
    ...
    text("vel " + str(vel), 0, fontSize);
    text("acc " + str(acc), 0, 2*fontSize);
    text("averagedDistance " + str(averagedDistance), 0, 3*fontSize);
    text("smoothedPixelPos " + str(smoothedPixelPos), 0, 4*fontSize);
    text("rawPixelPos " + str(rawPixelPos), 0, 6*fontSize);
    text("health " + str(health), 0, 8*fontSize);
